...
// Create a function called 'lovesCodeChecker' that takes in a single parameter called x. Check to see if x is equal to true. If it is, return true 
// as a boolean. If x does not equal true, return false as a boolean
...
let me = {
	...
	state: 'Utah',
	greeter: function (){
		return `Hello! ${firstName} and I live in ${state}`
	}
}
...
let total =  myNumbers.reduce((arr, num) =>  arr + num)
...
let myNumbersIndex = []
myNumbers.forEach((item) => myNumbersIndex.push(item))
...
let enemies = people.filter(key => {return key.friend === false})
...
let totallyAwesome = people.reduce((key, numb) => {return key + numb.awesomeLevel}, 0)
